...
settings <- list() %>%
  ...
  modifyList(list(
    ...
    ensemble = list(size = 20, variable = "NPP")
  ))
...
runs <- list_runs(con, workflow_id)
ncfiles <- run_dap(workflow_id, "2004.nc", runs$id)

read_ncvar <- function(file) {
...
ens_dat <- purrr::map_dfr(ncfiles, read_ncvar, .id = "id")
...
library(ggplot2)
ggplot(ens_sum) +
  aes(x = time, y = mean, ymin = lo, ymax = hi) +
