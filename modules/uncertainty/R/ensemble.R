  ...
  if (is.null(restart)){
    ...
    if (!is.null(con)){
      ...
      required_tags <- dplyr::tbl(con, "models") %>%
        ...
        dplyr::inner_join(dplyr::tbl(con, "modeltypes_formats"), by = "modeltype_id") %>%
        dplyr::collect() %>%
    ...
    for (i in seq_along(samp.ordered)) {
      ...
      samples[[names(samp.ordered[i])]] <- input.ens.gen(
        ...
        # if I have parent then give me their ids - this is where the
        # ordering matters making sure the parent is done before it's
        # asked
    ...
    # first element in params
    if (is.null(samp$parameters))            samples$parameters$samples <- ensemble.samples %>% purrr::map(~.x[rep(1, settings$ensemble$size) , ])
    # This where we handle the parameters - ensemble.samples is
    # already generated in run.write.config and it's sent to this
    # function as arg -
